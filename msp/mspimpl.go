...
// host name, the validation would fail with an sm2.CertificateInvalidError
// and a rason of sm2.NameConstraintsWithoutSANs.
func verifyLegacyNameConstraints(chain []*sm2.Certificate) error {
	if len(chain) < 2 {
		return nil
	}
	...
	for _, c := range chain[1:] {
		if oidInExtensions(oidExtensionNameConstraints, c.Extensions) {
			// return sm2.CertificateInvalidError{Cert: chain[0], Reason: x509.NameConstraintsWithoutSANs}
			return sm2.CertificateInvalidError{Cert: chain[0], Reason: sm2.CANotAuthorizedForThisName}
		}
	}
	return nil
}
...
func (msp *bccspmsp) sanitizeCert(cert *sm2.Certificate) (*sm2.Certificate, error) {
	...
	if isECDSASignedCert(cert) {
		...
		if cert.IsCA && cert.CheckSignatureFrom(cert) == nil {
			...
			isRootCACert = true
			validityOpts.Roots = sm2.NewCertPool()
