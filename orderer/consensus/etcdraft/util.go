...
func VerifyConfigMetadata(metadata *etcdraft.ConfigMetadata, verifyOpts sm2.VerifyOptions) error {
	if metadata == nil {
...
func parseCertificateFromBytes(cert []byte) (*sm2.Certificate, error) {
	...
	if pemBlock == nil {
		return &sm2.Certificate{}, errors.Errorf("no PEM data found in cert[% x]", cert)
	}
...
func parseCertificateListFromBytes(certs [][]byte) ([]*sm2.Certificate, error) {
	var certificateList []*sm2.Certificate
...
func createX509VerifyOptions(ordererConfig channelconfig.Orderer) (sm2.VerifyOptions, error) {
...
func validateConsenterTLSCerts(c *etcdraft.Consenter, opts sm2.VerifyOptions, ignoreExpiration bool) error {
	...
	verify := func(certType string, cert *sm2.Certificate, opts sm2.VerifyOptions) error {
